...
public static class Program
{
    public static async Task Main(string[] args)
    {
        ...
        // Virtual machine resource identifiers to delete. All VMs must be in the same subscription.
        List<ResourceIdentifier> resourceIds = HelperMethods.BuildVmResourceIds(
            subscriptionId,
            resourceGroupName,
            new[] { "dummy-vm-600", "dummy-vm-611", "dummy-vm-612" });

        await BulkActionsOperations.BulkDeleteOperationAsync(resourceGroupResource, executionParams, resourceIds, forceDeletion);
    }
}
